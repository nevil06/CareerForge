  ...
  return (
    <AppShell>
      <div className="max-w-3xl">
        <PageHeader
          eyebrow="Hiring signal feed"
          title="Notifications"
          description="Candidate match events and job activity updates."
        />
        {notifs.length === 0 ? (
              ...
              <Card key={n.id} className={clsx(!n.is_read && "border-fern/30 bg-fern/10")}>
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-xs font-semibold text-stone-400">{new Date(n.created_at).toLocaleDateString()}</p>
                  </div>
                </div>
              </Card>
            ))}
